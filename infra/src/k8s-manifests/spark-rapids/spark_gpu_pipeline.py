...
Estratégia de memória: materializa o silver como parquet no MinIO
antes dos gold. Cada gold query lê do silver materializado (já
filtrado, menos colunas) em vez de re-escanear os 12 parquets raw.
"""
import sys
import time
...
SILVER_PATH = "s3a://landing/spark-silver/trips_clean"
NUM_MONTHS = int(sys.argv[1]) if len(sys.argv) > 1 else 12

spark = SparkSession.builder.appName("fhvhv_gpu_pipeline").getOrCreate()
t0 = time.time()

# ---------------------------------------------------------------------------
# Bronze — lazy union de parquets
# ---------------------------------------------------------------------------
print(f"=== BRONZE ({NUM_MONTHS} months) ===")
dfs = []
for m in range(1, NUM_MONTHS + 1):
    df = spark.read.parquet(
        f"s3a://landing/fhvhv-2023/fhvhv_tripdata_2023-{m:02d}.parquet"
    )
...
print(f"  bronze registered ({time.time() - t0:.1f}s)")

# ---------------------------------------------------------------------------
# Silver — materializa como parquet (GPU processa, escreve no MinIO)
# ---------------------------------------------------------------------------
print("=== SILVER ===")
t1 = time.time()
spark.sql("""
SELECT
    pickup_datetime AS pickup_at, dropoff_datetime AS dropoff_at,
    ...
    CASE WHEN base_passenger_fare > 0
         ...
         ELSE 0.0 END AS driver_pct_of_fare
...
print(f"  silver materialized ({time.time() - t1:.1f}s)")

# ---------------------------------------------------------------------------
# Gold — lê do silver materializado (menos dados, menos RAM)
# ---------------------------------------------------------------------------
print("=== GOLD ===")
silver = spark.read.parquet(SILVER_PATH)
silver.createOrReplaceTempView("trips_clean")

# Enriched view pra borough_pairs
silver_enriched = spark.sql("""
SELECT t.*, pu.Borough AS pickup_borough, pu.Zone AS pickup_zone,
...
""")
silver_enriched.createOrReplaceTempView("trips_enriched")

gold_models = {
    "daily_revenue": """
        SELECT CAST(pickup_at AS DATE) AS trip_date, COUNT(*) AS total_trips,
        ...
        FROM trips_clean GROUP BY trip_date""",
    "hourly_demand": """
        SELECT DAYOFWEEK(pickup_at) AS weekday, HOUR(pickup_at) AS hour_of_day,
            COUNT(*) AS trips, ROUND(AVG(trip_minutes), 2) AS avg_trip_min,
        ...
        FROM trips_clean GROUP BY weekday, hour_of_day""",
    "borough_pairs": """
    ...
    "driver_economics": """
        SELECT DATE_TRUNC('month', pickup_at) AS month, COUNT(*) AS total_trips,
        ...
        FROM trips_clean GROUP BY month""",
    "shared_vs_solo": """
        SELECT CASE WHEN shared_matched THEN 'shared_matched'
                    WHEN shared_requested THEN 'shared_unmatched'
                    ELSE 'solo' END AS ride_type,
            COUNT(*) AS trips, ROUND(AVG(total_amount), 2) AS avg_total_usd,
            ROUND(AVG(trip_miles), 2) AS avg_miles,
            ROUND(AVG(trip_minutes), 2) AS avg_minutes,
            ROUND(AVG(driver_pay), 2) AS avg_driver_pay_usd,
            ROUND(SUM(total_amount), 2) AS total_revenue_usd
        FROM trips_clean GROUP BY ride_type""",
}

for name, query in gold_models.items():
    gt = time.time()
    spark.sql(query).write.mode("overwrite").parquet(f"{OUTPUT_BASE}/{name}/")
    print(f"  {name} ({time.time() - gt:.1f}s)")
